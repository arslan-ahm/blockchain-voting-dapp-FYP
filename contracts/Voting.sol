...
contract Voting is Ownable, ReentrancyGuard, KeeperCompatibleInterface {
    ...
    event CampaignCreated(uint256 campaignId, uint256 startDate, uint256 endDate, bool showVoters, bool showParticipants);
    ...
    campaign.showVoters = _showVoters;
    campaign.showParticipants = _showParticipants;
    
    emit CampaignCreated(nextCampaignId, _startDate, _endDate, _showVoters, _showParticipants);
    nextCampaignId++;
}
    ...
    function performUpkeep(bytes memory performData) external override {
        ...
        _endCampaign(campaignId);
    }

    function _endCampaign(uint256 _campaignId) internal {
        Campaign storage campaign = campaigns[_campaignId];
        require(campaign.isOpen, "Campaign already closed");
        
        campaign.isOpen = false;
        address winner = _calculateWinner(_campaignId);
        campaign.winner = winner;
    }

    function _calculateWinner(uint256 _campaignId) internal view returns (address) {
        Campaign storage campaign = campaigns[_campaignId];
    ...
    function getCampaignDetails(uint256 _campaignId) public view returns (
        ...
        return (
            ...
            campaign.showVoters,
            campaign.showParticipants,
            campaign.showVoters && msg.sender == admin ? campaign.voterList : new address[](0),
            campaign.showParticipants || msg.sender == admin ? campaign.candidateList : new address[](0)
        );
    }
